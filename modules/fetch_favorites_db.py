"""
抖音收藏列表 - SQLite 版 (增量抓取)
与 fetch_likes_db.py 共享同一数据库，通过 in_favorites 标记区分
同一视频只存一条记录，如果已存在于点赞中，只追加 in_favorites=1 标记
"""
...
def main():
    parser = argparse.ArgumentParser(description="抖音收藏列表 - SQLite 增量抓取")
    ...
    print(f"数据库已有: {existing_count} 条收藏视频, {author_count} 位作者")
    ...
    print(f"收藏视频: {total_videos} 条, 作者: {total_authors} 位")
    print("=" * 60)
